...
```bash
source .env.Codex && PGPASSWORD="$COW_DB_PASSWORD" psql \
...
Use `$ETH_MAINNET_RPC` from `.env.Codex` for mainnet. Use `cast` or whatever tools you want freely.
...
Use MCP `mcp__fetch__fetch` tool. API Key in `.env.Codex` as `$ETHERSCAN_API_KEY`.
...
Read the API key from `.env.Codex` and use it directly in the URL (MCP fetch doesn't do shell variable substitution).
